...
def build_dependency_dict(config: EngineConfig) -> dict[str, Any]:
    """Return a machine-readable dependency graph as nested dicts."""
    graph: dict[str, Any] = {"orchestrator": {}}
    for agent_id, agent in config.agents.items():
        agent_entry: dict[str, Any] = {
            "description": agent.description,
            "required_pipeline": agent.required_pipeline,
            "subagents": {},
        }
        for sub_id in agent.dependencies:
            sub = config.subagents.get(sub_id)
            if sub is None:
                agent_entry["subagents"][sub_id] = {"error": "missing config"}
                continue
            agent_entry["subagents"][sub_id] = {
