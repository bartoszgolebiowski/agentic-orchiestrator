...
class SubagentExecutor:
    ...
    async def _build_mcp_tool_context(self) -> tuple[list[dict[str, Any]], list[str], list[str], set[str]]:
        ...
        # Build HITL-skip set from mcp_skip_hitl_tools config
        skip_hitl_ids: set[str] = set()
        if self.config.mcp_skip_hitl_tools:
            for server_id, skip_names in self.config.mcp_skip_hitl_tools.items():
                discovered = {d.remote_name for d in descriptors if d.server_id == server_id}
                missing = set(skip_names) - discovered
                if missing:
                    raise ValueError(
                        f"Subagent '{self.config.id}' mcp_skip_hitl_tools references "
                        f"unknown tools from server '{server_id}': {sorted(missing)}"
                    )
            for descriptor in descriptors:
                server_skip = self.config.mcp_skip_hitl_tools.get(descriptor.server_id)
    ...
    async def execute(self, task: str, input_json: Any | None = None) -> str:
        ...
        mcp_tool_specs, mcp_descriptions, mcp_allowed_ids, mcp_skip_hitl = await self._build_mcp_tool_context()
        ...
        async def handle_action(name: str, arguments: dict[str, Any]) -> str:
            ...
            if name in mcp_allowed_ids:
                ...
                if self.hitl_callback is not None and name not in mcp_skip_hitl:
                    pending = PendingToolCall(
                        tool_name=name,
                        arguments=arguments,
                        tool_call_id=f"{self.config.id}:{name}:{id(arguments)}",
                        subagent_id=self.config.id,
                        source="mcp",
                    )
                    emit_event(
                    ...
                    hitl_response = await self.hitl_callback(pending)
                    emit_event(
                    ...
                    if not hitl_response.approved:
                        reason = hitl_response.rejection_reason or "rejected by human"
                        return f"TOOL REJECTED: {reason}"
                    if hitl_response.modified_arguments is not None:
                        arguments = hitl_response.modified_arguments
