...
def load_engine_config(config_dir: Path) -> EngineConfig:
    ...
    # Validate critical cross-references (missing references are non-recoverable)
    for agent_id, agent in agents.items():
        for dep in agent.dependencies:
            if dep not in subagents:
                raise ValueError(
                    f"Agent '{agent_id}' references unknown subagent '{dep}'"
                )
    for sub_id, sub in subagents.items():
        for mcp_dep in sub.mcp_dependencies:
            if mcp_dep not in mcps:
                raise ValueError(
